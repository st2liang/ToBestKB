...
def conversation_create(request):
    if request.method == 'POST':
        if request.user.is_authenticated:
            ...
            return JsonResponse({'conversationid': dialog.id}, status=200)
        else:
...
def conversation_content(request):
    if request.method == 'GET':
        ...
        content ,matched_text = "LLM中封装好的函数（参数为query，和可选向量库，可选历史内容）",query
        """
        此处为大模型中的函数，接受一个必需项query，两个可选项向量库地址，历史内容
        返回回答内容
        """
        dialog = Dialog.objects.get(id=conversation_id)
        history = dialog.content
        if history is None:
            history=[]
        history.append([query,content])
        dialog.content = history
        dialog.save()


        return JsonResponse({"match_text:" : matched_text, "answer": content},status=200)
    else:
