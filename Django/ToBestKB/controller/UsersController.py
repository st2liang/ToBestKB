...
def sign_up(request):
    if request.method == 'POST':
        try:
            ...
            import random
            name = "User_"+str(random.randint(1,10000))

            user = User.objects.create_user(name=name, account=account,password=password)
            return JsonResponse({ 'returnid': user.id},status=200)
...
def user_login(request):
    if request.method == 'POST':
        try:
            ...
            if user is not None:
                login(request, user)
                return JsonResponse({"return_id": user.id},status=200)
            else :
